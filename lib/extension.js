...
function processor(resource, content) {
    ...
    this.analyzer.checked = this.analyzer.checked || {};
    if (!/javascript/.test(type) || this.analyzer.checked[resource.path]) {
        return;
    }
    this.analyzer.checked[resource.path] = true;
    var result = this.checker.check(content, resource.path);
    if (!result.ok) {
        // cacheable
        var path = resource.path;
        if (result.errors[0].type === syntax.SYNTAX_ERROR) {
            this.analyzer.fatal("Syntax error in " + path, result);
        } else {
            this.analyzer.error("ReferenceError in " + path, result);
        }
    }
}
